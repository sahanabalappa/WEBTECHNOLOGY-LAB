# WEBTECHNOLOGY
#### Program 1
###### Javascript Program calculates the squares and cubes of the numbers from 0 to 10 and outputs HTML text that displays the resulting values in an HTML table format.
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/tree/master/program1
#### program 2
###### Program to Design of Simple Calculator to perform addtion,subtration,multiplication and division using JavaScript. 
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/blob/master/Program2/calculator.html
#### program 3
###### Javascript program that displays Text Growing in red color and shrinking in blue color. 
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/blob/master/Program3.html
#### Program 4
###### HTML5 file that includes javaScript script that uses functions for the following problems:
###### a)Parameter: A String 
###### b)output:The position inthe string of the left-most vowel
###### c)Parameter:A number
###### d) Output:The number with its digits in the reverse order
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/tree/master/program4
#### Program 5
###### XML document to store information about a student in an engineering college affiliated to VTU. The information must include USN, Name, and Name of the College, Branch, Year of Joining, and email id. Make up sample data for 3 students. Create a CSS style sheet and use it to display the document.
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/blob/master/program5
#### Program 6
###### PHP program to keep track of the number of visitors visiting the webpages and to display the webpage and display the count of visitors with proper heading.
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/tree/master/program6
#### Program 7
###### PHP program to display a digital clock which displays the current time of the server
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/tree/master/program7
#### Program 8
###### PHP Program to 
###### a)Implement Simple calculator Operations 
###### b)Find the transpose of a matrix 
###### c)Multiplication of Two Matrices
###### d)Addition of Two matrices
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/tree/master/program8
#### Program 9
###### PHP program named states.php that declares a variable states with value "Mississipi Alabama Texas Massachusetts Kansas" That does the following 
###### a)Search for a word in variable states that ends with xas.store this word in element 0 of a list named statesArray/statesList.
###### b)search for the word in states that begins with K and ends with s.perform case insensitive comparision,store this word in element1 of stateslist.
###### c)search for the word in states that begins with M and ends with s,store this word in element2 of stateslist.
###### d)search for the word in states that ends in a,store this word in element3 of stateslist.
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/tree/master/program9
#### Program 10 
###### Write a PHP program to sort the student records which are stored in the database using selection sort.
https://github.com/sahanabalappa/WEBTECHNOLOGY-LAB/tree/master/program10


